...
import cn.com.u2be.myutils.NetworkUtil;
import cn.com.u2be.myutils.StringUtil;
...
public class ClientSocket implements Runnable {
    ...
    public ClientSocket(final Context context, final CmdParser cmdParser, final String ip, int port, Handler recvHandler) {
    ...
    public void run() {
        if (!IsParamOk)
            return;
        if (!NetworkUtil.CheckWifi(mContext)) {
            logger.i("WIFI不可用");
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        OnReceive(ConnMsgType.Conn, ConnState.DisConnected, "连接结束!");
                    }

...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends ViewGroup implements ChartInterface {
    ...
    public void highlightTouch(Highlight high) {
        ...
        if (high == null) {
            ...
            entries = mData.getEntryForHighlight(high);
            if (entries == null || entries.size() == 0 || entries.get(0).getXIndex() != high.getXIndex()) {
                mIndicesToHightlight = null;
                high = null;
            } else {
                // set the indices to highlight
        ...
        if (mSelectionListener != null) {
            ...
            if (!valuesToHighlight())
                ...
                // notify the listener
                mSelectionListener.onValueSelected(entries, high.getDataSetIndex(), high);
            }
        }
    }
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHightlight.length; i++) {
            ...
            if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
                ...
                List<Entry> entries = mData.getEntryForHighlight(mIndicesToHightlight[i]);

                // make sure entry not null
                if (entries == null || entries.size() < 1 || entries.get(0).getXIndex() != mIndicesToHightlight[i].getXIndex())
                    continue;
                ...
                for (Entry e : entries) {
                    ...
                    mMarkerView.refreshContent(entries, highlight);
    ...
    protected void drawSpecialMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHightlight.length; i++) {
            ...
            if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
                ...
                // make sure entry not null
                if (entries == null || entries.size() == 0 || entries.get(0).getXIndex() != mIndicesToHightlight[i].getXIndex())
                    continue;
                for (Entry entry : entries) {
                    float[] pos = getMarkerPosition(entry, highlight);
                    ...
                    mMarkerView.refreshContent(entries, highlight, xVals.get(entry.getXIndex()));
                    mMarkerView.refreshContent(mIndicesToHightlight[i].getXIndex(), markerIndex, mData.getDataSets());

                    mMarkerView.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
                    mMarkerView.layout(0, 0, mMarkerView.getMeasuredWidth(), mMarkerView.getMeasuredHeight());
                    float mMarkerViewX = pos[0];
                    float offset = (mMarkerView.getWidth() / 2);
                    if (pos[0] < getRight() / 5) {
                        mMarkerViewX = offset + pos[0];
                    }
                    if (pos[0] > getRight() / 5 * 4) {
                        mMarkerViewX = pos[0] - offset;
                    }
                    mMarkerView.draw(canvas, mMarkerViewX, mMarkerView.getHeight() + Utils.convertDpToPixel(15f));


                    // mMarkerView.draw(canvas, pos[0], 0);

                    Paint paint = new Paint();
                    paint.setColor(getSpecialMarkerLineColor()[0]);
                    ...
                    Bitmap specialMarkerCircle = getSpecialMarkerCircle()[0];
     ...
     * necessary to refresh the chart. ANIMATIONS ONLY WORK FOR API LEVEL 11 (Android 3.0.x) AND HIGHER.
     */
    public void animateXStop() {
        mAnimator.animateXStop();
    }
